...
(function(){ //[
 ...
 ipfs.getPeerId = getPeerId = function getPeerId() {
...
getPeerId().then(id => { return ipfs.peerid = id }).catch(console.warn);
...
ipfsPeerConnect(ipfs.bootstrapNode)
 .then( status => {
...
ipfs.mfsGetHashByPath = mfsGetHashByPath = getMFSFileHash = function getMFSFileHash(mfspath) {
   let [callee, caller] = essential.functionNameJS(); // logInfo("message !")
   console.debug(callee+'.inputs:',{mfspath});
